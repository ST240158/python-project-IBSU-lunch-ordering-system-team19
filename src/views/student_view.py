    ...
    def show_menu_table(self, items: list[dict], categories: list[dict] | None = None) -> None:
        ...
        if categories:
            for cat in categories:
                ...
                if cat_items:
                    rows = [(i["food_item_id"], i["name"], i["description"] or "", f"K{i['price']:.2f}", i["stock"]) for i in cat_items]
                    print(f"\n  📂 {cat['name']}")
                    print(tabulate(rows, headers=["ID", "Name", "Description", "Price", "Stock"], tablefmt="fancy_grid"))
        else:
            rows = [(i["food_item_id"], i["name"], i["description"] or "", f"K{i['price']:.2f}", i["stock"]) for i in items]
            print("\n  🍽️  Menu")
            print(tabulate(rows, headers=["ID", "Name", "Description", "Price", "Stock"], tablefmt="fancy_grid"))
        print()
