        ...
        try:
            ...
            if not sales_df.empty:
                for _, row in sales_df.iterrows():
                    text_lines.append(f"  Order #{row['order_id']}: {row['student_name']} — ${row['total_amount']:.2f} ({row['status']})")
                text_lines.append(f"  Total revenue: ${sales_df['total_amount'].sum():.2f}")
            else:
            ...
            if not popular_df.empty:
                for _, row in popular_df.iterrows():
                    text_lines.append(f"  {row['name']}: {row['total_qty']} sold (${row['revenue']:.2f})")
            else:
            ...
            if not cat_df.empty:
                for _, row in cat_df.iterrows():
                    text_lines.append(f"  {row['category_name']}: ${row['total_revenue']:.2f} ({row['order_count']} orders)")
            else:
                text_lines.append("  No data.")
