...
class AdminView:
    ...
    def show_food_items_table(self, items: list[dict], title: str = "🍽️  Menu Items") -> None:
        ...
        if not items:
            print(f"\n  {title}\n  (No items to display.)\n")
            return
        rows = [(i["food_item_id"], i["category_id"], i["name"], i.get("description", ""), f"K{i['price']:.2f}", i["stock"]) for i in items]
        print(f"\n  {title}")
    ...
    def show_orders_table(self, orders: list[dict], title: str = "📋 All Orders") -> None:
        ...
        if not orders:
            ...
            return
        rows = [
            (o["order_id"], o.get("student_name", ""), o.get("order_date", ""), f"K{o['total_amount']:.2f}", o.get("status", ""))
            for o in orders
        ]
        print(f"\n  {title}")
    ...
    def show_report_submenu(self) -> str:
        ...
        print("  6. External Service Check")
        print("  7. Back")
    ...
    def show_external_service_result(self, result: dict) -> None:
        """Display the result of the external time-service request."""
        print(self.THIN)
        print("  EXTERNAL SERVICE CHECK")
        print(self.THIN)
        print(f"  Service:       {result['service']}")
        print(f"  HTTP Status:   {result['status_code']}")
        print(f"  Location:      {result['timezone']}")
        print(f"  Current Time:  {result['datetime']}")
        print(f"  UTC Offset:    {result.get('utc_offset', 'N/A')}")
        print("  Response:      Service available")
        print(self.THIN)